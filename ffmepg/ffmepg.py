...
layout = [
    ...
    [sg.Button("查询支持解码的环境")],
    [sg.Combo(["", "-c:v h264_cuvid", "-hwaccel cuda", "-hwaccel_device 0 -hwaccel cuda"], key="解码")],
]
...
while True:
    ...
    if event == "开始转换":
        ...
        if filename and format:
            # 生成输出文件名
            output = (
                values["-FOLDER-"] + "/" + filename.rsplit(".", 1)[0] + "." + format
            )
            # 调用ffmpeg进行视频转换
            cmd = ["ffmpeg", jiema,"-i", filename, output]
            subprocess.run(cmd)
            # 弹出对话框提示转换完成
